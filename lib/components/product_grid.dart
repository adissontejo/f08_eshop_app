        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            List<Product> products = snapshot.data!
                .where(
                  (item) =>
                      (item.creatorUserId == userList.loggedUser!.id ||
                          !showOnlyOwned) &&
                      (item.isFavorite || !showOnlyFavoritos),
                )
                .toList();
            return ProductGridView(
              products: products,
              editorView: showOnlyOwned,
            );
          } else {
            return Text(
...
class ProductGridView extends StatelessWidget {
  const ProductGridView(
      {super.key, required this.products, this.editorView = false});
